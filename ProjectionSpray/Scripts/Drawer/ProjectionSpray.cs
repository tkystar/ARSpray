...
public class ProjectionSpray : DrawerBase
{
    new Camera camera
    {
        get
        {
            if (_cam == null)
            {
                ...
                depthOutput = new RenderTexture(dpthResolution, dpthResolution, 0, RenderTextureFormat.RFloat);   //renderTextureを用意
                depthOutput.Create();　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　//renderTextureを作成
    ...
    protected override void UpdateDrawTarget()
    {
        ...
        var planes = GeometryUtility.CalculateFrustumPlanes(camera);   // 視錐台平面を獲得
        drawTargetList.AddRange(
            DrawableBase.AllDrawableList.FindAll(d => GeometryUtility.TestPlanesAABB(planes, d.bounds))  // 内外判定
            );
    }
